jQuery.fn.calendarPicker = function(options) {
  ...
  theDiv.addClass("calBox clearfix");
  ...
  var divDays = $("<div>").addClass("calDay col-xs-12");
    ...
    var fillDays = function(date) {
      ...
      for (var i = -options.days; i <= options.days; i++) {
        ...
        if (i == -options.days && options.showDayArrows) {
          span.addClass("prev col-xs-1 fa fa-chevron-left");
        } else if (i == options.days && options.showDayArrows) {
          span.addClass("next col-xs-1 fa fa-chevron-right");
        } else {
          span.addClass("col-xs-2");
          span.html(options.dayNames[d.getDay()] + '<br/>' + "<span class=dayNumber>" + options.monthNames[d.getMonth()] + '/' + d.getDate() + "</span>").css("width", w * 5);
